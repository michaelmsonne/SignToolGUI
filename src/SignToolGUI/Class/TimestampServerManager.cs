...
{
    public class TimestampServerManager
    {
        ...
        public TimestampServerManager(string displayName, string url, bool isEnabled = true, int priority = 0, int timeoutSeconds = 30)
        {
    ...
    {
        private readonly List<TimestampServerManager> _timestampServers;
        private readonly List<TimestampServerManager> _defaultServers; // Store default servers
        private readonly List<TimestampServerManager> _trustedSigningServers; // Store trusted signing servers
        ...
        {
            _timestampServers = new List<TimestampServerManager>();
            _defaultServers = new List<TimestampServerManager>();
            _trustedSigningServers = new List<TimestampServerManager>();
        ...
        private void InitializeServerCollections()
        {
            ...
            _defaultServers.Add(new TimestampServerManager("Sectigo (Comodo)", "http://timestamp.sectigo.com", true, 1, 30));
            _defaultServers.Add(new TimestampServerManager("DigiCert", "http://timestamp.digicert.com", true, 2, 30));
            _defaultServers.Add(new TimestampServerManager("GlobalSign (1)", "http://timestamp.globalsign.com/tsa/r6advanced1", true, 3, 30));
            _defaultServers.Add(new TimestampServerManager("GlobalSign (2)", "http://timestamp.globalsign.com/?signature=sha2", true, 4, 30));
            _defaultServers.Add(new TimestampServerManager("Certum", "http://time.certum.pl", true, 5, 30));

            // Initialize trusted signing servers collection
            _trustedSigningServers.Clear();
            _trustedSigningServers.Add(new TimestampServerManager("West Europe", "https://weu.codesigning.azure.net", true, 1, 30));
            _trustedSigningServers.Add(new TimestampServerManager("North Europe", "https://neu.codesigning.azure.net", true, 2, 30));
            _trustedSigningServers.Add(new TimestampServerManager("West US 2", "https://wus2.codesigning.azure.net", true, 3, 30));
            _trustedSigningServers.Add(new TimestampServerManager("West Central US", "https://wcus.codesigning.azure.net", true, 4, 30));
            _trustedSigningServers.Add(new TimestampServerManager("East US", "https://eus.codesigning.azure.net", true, 5, 30));
        }
        ...
        public void SetDefaultServers()
        {
            ...
            {
                _timestampServers.Add(new TimestampServerManager(server.DisplayName, server.Url, server.IsEnabled, server.Priority, server.TimeoutSeconds));
            }
        ...
        {
            var server = new TimestampServerManager(displayName, url, isEnabled, priority, timeoutSeconds);
        ...
        public List<TimestampServerManager> GetServers()
        {
            return _timestampServers.ToList();
        }

        public List<TimestampServerManager> GetEnabledServers()
        {
        ...
        public async Task<bool> TestServerAsync(TimestampServerManager server)
        {
            try
            {
        ...
        public async Task<List<TimestampServerManager>> GetAvailableServersAsync()
        {
            var enabledServers = GetEnabledServers();
            var availableServers = new List<TimestampServerManager>();
        ...
        public void ReplaceAllServers(List<TimestampServerManager> newServers)
        {
            _timestampServers.Clear();
